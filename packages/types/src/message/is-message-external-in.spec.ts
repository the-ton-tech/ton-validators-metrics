import { Cell, loadTransaction } from "@ton/core";
import { isMessageExternalIn } from "./is-message-external-in";

describe("is-message-external-in", (): void => {
  it("should check message external in", (): void => {
    const transactionBoc = ``;
    const transactionCell = Cell.fromBase64(transactionBoc);
    const transaction = loadTransaction(transactionCell.beginParse());

    expect(isMessageExternalIn(transaction.inMessage)).toBe(false);
  });
});
